...
def tag_bloco(conteudo, *args, classe='success', inline=False, **novo_atrs):
    # atrs é mesma coisa que atributos
    tag = 'span' if inline else 'div'
    html = conteudo if not callable(conteudo) else conteudo(*args, **novo_atrs)
    atributos = filtrar_atrs(novo_atrs, bloco_atrs)
    return f'<{tag} {atributos} class="{classe}">{html}</{tag}>'


def tag_lista(*itens, **novo_atrs):
    lista = ''.join((f'<li>{item}</li>' for item in itens))
    return f'<ul{filtrar_atrs(novo_atrs, ul_atrs)}>{lista}</ul>'


if __name__ == '__main__':
    print(tag_bloco(tag_lista('Item 1', 'Item 2', 'Item 3'), classe='Lista'))
    print(tag_bloco(tag_lista, 'Sábado', 'Domingo',
                    classe='info', inline=True))
    print(tag_bloco(tag_lista, 'Item 1', 'Item 2', classe='info',
                    bloco_accesskey='m', bloco_id='conteúdo', us_id='lista'))
