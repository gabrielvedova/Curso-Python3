...
def main():
    ...
    casa.add('Lavar a louça', datetime.now() + timedelta(days=3, seconds=1))
    casa.add('Lavar o carro')
    casa += TarefaRecorrente('Trocar lençóis', datetime.now(), 7)
    casa.procurar('Trocar lençóis').concluir()
    print(casa)

    try:
        casa.procurar('Guardar roupa - ERRO').concluir
    except TarefaNaoEncontrada as e:
        print(f'A causa foi "{str(e)}"!')

    casa.procurar('Lavar o carro').concluir()
    for tarefa in casa:
        print(f'- {tarefa}')
    print(casa)
    ...
    mercado.add('Biscoitos')
    print(mercado)

    comprar_verduras = mercado.procurar('Verduras')
    comprar_verduras.concluir()
